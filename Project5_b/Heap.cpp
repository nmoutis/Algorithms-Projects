...
int heapV<T,U>::getIndex(T &key) // Return the index of the item key in the heap.  Assumes each item is unique.
{
...
template <typename T, typename U>
heapV<T,U>::heapV()// Construct an empty heap.
{
}

template <typename T, typename U>
void heapV<T,U>::initializeMaxHeap(vector<T> list2, U &g) // Initialize a max heap from values in list2.
{
...
template <typename T, typename U>
void heapV<T,U>::initializeMinHeap(vector<T> list2, U &g) // Initialize a min heap from values in list2.
{
    for (int i = 0; i < list2.size(); i++)
        list.push_back(list2[i]);
...
// lets the value at i float down so the subtree rooted at i obeys the max heap property.
{
    ...
    // Set largest = largest of the two children and i (the root)
    ...
    // If root isn't the largest, swap the root with the largest child and recurse.
    if (largest != i)
    {
...
// lets the value at i float down so the subtree rooted at i obeys min heap property.
{
    int l = left(i);
    int r = right(i);
    int smallest;
    
    // Set smallest = smallest of the two children and i (the root)
...
template <typename T, typename U>
T heapV<T,U>::getMinHeapMinimum() // Return the smallest item in a min heap.
{
    return list[0];
}

template <typename T, typename U>
T heapV<T,U>::getMaxHeapMaximum() // Return the largest item in a max heap
{
    return list[0];
}

template <typename T, typename U>
T heapV<T,U>::extractMinHeapMinimum(U &g) // Return the smallest item in a min heap and remove it.
{
    if (size() < 1)
        throw rangeError("heap too small in extractMinHeapMinimum");
    T min = list[0];
    list[0] = list[size()-1];
    //cout << "Moving " << list[0] << " to the top" << endl;
    list.resize(size()-1);
    minHeapify(0,size(),g);
    return min;
}
...
template <typename T, typename U>
T heapV<T,U>::extractMaxHeapMaximum(U &g) // Return the largest item in a max heap and remove it.
{
    if (size() < 1)
...
template <typename T, typename U>
void heapV<T,U>::maxHeapInsert(T key, U &g) // Adds a new leaf then calls maxHeapIncreaseKey to move the key to the correct location.
{
    list.push_back(key);
...
template <typename T, typename U>
void heapV<T,U>::minHeapInsert(T key, U &g) // Adds a new leaf then calls minHeapDecreaseKey to move the key to the correct location.
{
    list.push_back(key);
...
// Given an unordered list, first builds a max-heap.  The maximum element is now in the first position, and it is swapped into the
// last position.  The heap size is reduced by one, which might cause element 1 to violate the max-heap property.  The call to maxHeapify
...
template <typename T, typename U>
ostream &operator<<(ostream &ostr, heapV<T,U> &h) // Overload << to print all information about the graph.
{
    cout << "------------------------------------------------" << endl;
